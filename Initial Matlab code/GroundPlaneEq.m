...
for frame = 0:2
    % filename = srcImages(frame).name;
    % [pathstr,name,ext] = fileparts(filename);
    %
    % %read left image and its coresponding disparity map and labelled image
    % frameLeft = imread(strcat(dirImages,'/',filename));
    % frameDisp = imread(strcat(dirDisparity,'/',name,'_disp.png'));
    frameLeft = imread(strcat(dirImages,'/','left_', num2str(frame),'.png'));
    frameDisp = imread(strcat(dirDisparity,'/','disp_', num2str(frame),'.png'));
    gpImg = imread(strcat(dirGP, '/', 'gp_', num2str(frame), '.png'));
